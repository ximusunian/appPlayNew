 ...
 * @LastEditTime: 2020-12-01 14:06:04
-->
<template>
  <div>
    <van-tabbar v-model="tab_bar" active-color="#FF702C" inactive-color="#666666" :border=false :class="['bar-shadow', isShowSpecial?'safe-area':'']">
      ...
      <van-tabbar-item v-if="!showFake" name="enlightening" to="/enlightening">
        <span>收徒赚钱</span>
        <template #icon="props">
...
export default {
  ...
  data() {
    return {
      ...
      showFake: localStorage.getItem("showFake"),
      isShowSpecial: false
    };
  },
  created() {
  ...
  computed: {
    ...
    getFake() {
      return this.$store.state.showFake
    }
  },
  methods: {}
};
